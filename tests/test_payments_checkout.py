...
// Tests for checkout
// Bug fix: Fixed missing logging import causing NameError in checkout endpoint
...
describe('Checkout', () => {
  ...
  it('Should return error when order is already paid', () => {
    const order_id = 1;
    const result = checkout(order_id);
    expect(result).toBe({ error: 'Order is already paid' });
  });
